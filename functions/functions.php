...
function wpat_jqdt_sub_loader() {
  ...
  if ( $offset ) {
    ...
    foreach ( $subs as $i => $t ) {
      $userData = get_userdata($t->post_author);

      $subscriptions[$i]['0'] = $t->ID;
      $subscriptions[$i]['1'] = '<a href="' . get_bloginfo('wpurl') . '/wp-admin/user-edit.php?user_id=' . $userData->ID . '">' . $userData->user_login . '</a>';
      $subscriptions[$i]['2'] = get_post_time("F j, Y, g:i a",false,$t->ID);
      $subscriptions[$i]['3'] = get_post_meta($t->ID,'billingFirstName',true);
      $subscriptions[$i]['4'] = get_post_meta($t->ID,'billingLastName',true);
      $subscriptions[$i]['5'] = get_post_meta($t->ID,'billingEmail',true);
      $subscriptions[$i]['6'] = get_post_meta($t->ID,'billingCompany',true);
      $subscriptions[$i]['7'] = get_post_meta($t->ID,'billingPhoneNumber',true);
      $subscriptions[$i]['8'] = get_post_meta($t->ID,'billingAddress',true);
      $subscriptions[$i]['9'] = get_post_meta($t->ID,'billingCity',true);
      $subscriptions[$i]['10'] = get_post_meta($t->ID,'billingState',true);
      $subscriptions[$i]['11'] = get_post_meta($t->ID,'billingZip',true);
      $subscriptions[$i]['12'] = get_post_meta($t->ID,'billingCountry',true);
      $subscriptions[$i]['13'] = get_post_meta($t->ID,'shippingFirstName',true);
      $subscriptions[$i]['14'] = get_post_meta($t->ID,'shippingLastName',true);
      $subscriptions[$i]['15'] = get_post_meta($t->ID,'shippingCompany',true);
      $subscriptions[$i]['16'] = get_post_meta($t->ID,'shippingAddress',true);
      $subscriptions[$i]['17'] = get_post_meta($t->ID,'shippingCity',true);
      $subscriptions[$i]['18'] = get_post_meta($t->ID,'shippingState',true);
      $subscriptions[$i]['19'] = get_post_meta($t->ID,'shippingZip',true);
      $subscriptions[$i]['20'] = get_post_meta($t->ID,'shippingCountry',true);
      $subscriptions[$i]['21'] = str_replace('XXXX', '', get_post_meta($t->ID,'ccLastFour',true));
      $subscriptions[$i]['22'] = get_post_meta($t->ID,'subscriptionID',true);
      $subscriptions[$i]['23'] = get_post_meta($t->ID,'subscriptionStartDate',true);
      $subscriptions[$i]['24'] = get_post_meta($t->ID,'subscriptionReferenceID',true);
      $subscriptions[$i]['25'] = get_post_meta($t->ID,'subscriptionInvoiceNumber',true);
      $subscriptions[$i]['26'] = get_post_meta($t->ID,'subscriptionName',true);
      $subscriptions[$i]['27'] = get_post_meta($t->ID,'subscriptionInterval',true);
      $subscriptions[$i]['28'] = get_post_meta($t->ID,'subscriptionUnit',true);
      $subscriptions[$i]['29'] = get_post_meta($t->ID,'subscriptionOccurrences',true);
      $subscriptions[$i]['30'] = get_post_meta($t->ID,'subscriptionTrialOccurrences',true);
      $subscriptions[$i]['31'] = get_post_meta($t->ID,'subscriptionAmount',true);
      $subscriptions[$i]['32'] = get_post_meta($t->ID,'subscriptionTrialAmount',true);
      $subscriptions[$i]['33'] = get_post_meta($t->ID,'subscriptionLastBillingDate',true);
      $subscriptions[$i]['34'] = get_post_meta($t->ID,'subscriptionNextBillingDate',true);
      $subscriptions[$i]['35'] = get_post_meta($t->ID,'subscriptionPaymentNumber',true);
      $subscriptions[$i]['36'] = get_post_meta($t->ID,'subscriptionStatus',true);
      $subscriptions[$i]['37'] = get_post_meta($t->ID,'subscriptionCanceledBy',true);
      $subscriptions[$i]['38'] = get_post_meta($t->ID,'subscriptionCanceledDate',true);
    }
